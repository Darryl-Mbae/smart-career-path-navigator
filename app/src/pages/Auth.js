...
function Auth() {
  ...
  }, "className": "p-[1.25rem] rounded-[0.5rem] border-none bg-[#0b0b0b] text-white text-[16px] w-full mb-[16px] focus:border-primary focus:ring-1 focus:ring-primary", "autoComplete": "new-email"}, []), __jacJsx("input", {}, []), __jacJsx("div", {"className": "w-full relative"}, [__jacJsx("input", {"type": showPassword ? "text" : "password", "placeholder": "Password", "value": loginPassword, "onChange": e => {
    setLoginPassword(e.target.value);
  }, "className": "p-[1.25rem] rounded-[0.5rem] border-none bg-[#0b0b0b] text-white text-[16px] w-full focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary", "autoComplete": "new-password"}, []), __jacJsx("input", {}, []), PasswordToggleIcon({showPassword: showPassword, setShowPassword: setShowPassword})]), error && __jacJsx("p", {"className": "text-[#dc2626] mb-4 text-sm"}, [error]), __jacJsx("button", {"type": "submit", "className": "w-full p-5 rounded-[8px] border-none bg-primary text-white font-semibold cursor-pointer"}, [isLoading ? LoadingDots() : "Sign In"])]), __jacJsx("p", {"className": "mt-[19px] text-[14px] text-[#bbbbbb] items-center"}, ["Don't have an account?", __jacJsx("span", {"onClick": e => {
    setIsSignIn(false);
    setError("");
